...
export class Client {
    ...
    private constructor(options: ClientOptions, key: symbol) {
        ...
        if (
            this.options.token == '' ||
            this.options.token.length < tokenMinimalLength ||
            !this.options.token
        ) {
            throw new IncorrectBotOptions(
                'token',
                this.options.token,
                `Token minimal length is ${tokenMinimalLength}`,
            );
        }

        if (
            !this.options.logger?.level ||
            !LogLevels.includes(this.options.logger?.level)
        ) {
            throw new IncorrectBotOptions(
                'logger.level',
                this.options.logger?.level,
                `Logger level must be one of [${LogLevels.join(', ')}]`,
            );
        }

        if (
            this.options.updates?.method == 'webhook' &&
            !this.options.updates.webhook.domain
        ) {
            throw new IncorrectBotOptions(
                'updates.webhook.domain',
                this.options.updates.webhook.domain,
                `Webhook server needs domain e.g. https://*-*-*-*.trycloudflare.com or any other`,
            );
        }
    ...
    private async init(): Promise<void> {
        ...
        switch (this.options.updates?.method) {
            case 'longpoll':
                ...
                this.updates = new Longpoll(
                    this,
                    this.handleUpdate,
                    this.options.updates.longpoll!,
                );
            ...
            case 'webhook':
                ...
                this.updates = new Webhook(
                    this,
                    this.handleUpdate,
                    this.options.updates.webhook,
                );
    ...
    private handleUpdate = (update: Update) => {
        ...
        this.logger.debug({
            text: `<gray>${JSON.stringify(update)}</gray>`,
            module: 'updates',
        });

        for (const event in UpdateEventMap) {
            const payload =
                UpdateEventMap[event as keyof TelegramEventMap](update);
