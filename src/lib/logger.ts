...
export type ChalkFn = (msg: string) => string
...
export class Logger {
    ...
    private static applyStyles(message: string): string {
        return message
            .replace(/<([a-zA-Z]+)>(.*?)<\/\1>/gs, (_, tag: string, content: string) => {
                return this.applyChalkTag(tag, content)
            })
            .replace(/<(#[0-9a-fA-F]{6})>(.*?)<\/\1>/g, (_, tag: string, content: string) => {
                return this.applyHexTag(tag, content)
            })
            .replace(/<bg#([0-9a-fA-F]{6})>(.*?)<\/bg#\1>/g, (_, tag: string, content: string) => {
    ...
    private log(level: LogLevel, { text, module }: LogParams) {
        ...
        const prefix = levelColorMap[level](`[${Logger.getTimestamp()} - ${level.toUpperCase()}]`)
